...
@[simp]
theorem dvd_mul_right (a b : α) : a ∣ a * b := by
  aesop (add simp [dvd_def])

@[aesop 50%]
theorem dvd_mul_of_dvd_left (h : a ∣ b) (c : α) : a ∣ b * c := by
  aesop (add simp [dvd_def])
...
@[simp]
theorem one_dvd (a : α) : 1 ∣ a := by
  aesop (add simp [dvd_def])

theorem dvd_of_eq (h : a = b) : a ∣ b := by
  aesop
...
@[aesop 90%]
lemma dvd_pow (hab : a ∣ b) : ∀ {n : ℕ} (_ : n ≠ 0), a ∣ b ^ n := by
  intro n; induction n <;> aesop (add simp pow_succ')

alias Dvd.dvd.pow := dvd_pow

lemma dvd_pow_self (a : α) {n : ℕ} (hn : n ≠ 0) : a ∣ a ^ n := by
  aesop

theorem mul_dvd_mul_left (a : α) (h : b ∣ c) : a * b ∣ a * c := by
  aesop (add simp [dvd_def, mul_assoc])
...
theorem Dvd.intro_left (c : α) (h : c * a = b) : a ∣ b := by
  aesop (add safe Dvd.intro, simp [mul_comm])

alias dvd_of_mul_left_eq := Dvd.intro_left

attribute [aesop safe] mul_comm

theorem exists_eq_mul_left_of_dvd (h : a ∣ b) : ∃ c, b = c * a := by
  aesop (add simp [dvd_def])

theorem dvd_iff_exists_eq_mul_left : a ∣ b ↔ ∃ c, b = c * a := by
  aesop (add simp [dvd_def])
...
@[aesop 30%]
theorem mul_dvd_mul : ∀ {a b c d : α}, a ∣ b → c ∣ d → a * c ∣ b * d := by
  aesop (add simp [dvd_def])
